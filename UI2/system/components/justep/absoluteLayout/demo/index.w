...
<div xmlns="http://www.w3.org/1999/xhtml" xid="window" class="window" component="$UI/system/components/justep/window/window">  
  <div component="$UI/system/components/justep/model/model" xid="model" style="height:auto;left:2px;top:264px;"
    onLoad="loadData"> 
    <div component="$UI/system/components/justep/data/data" xid="todoData"
      idColumn="id" autoLoad="false" confirmDelete="false" onValueChanged="saveData"
      onAfterNew="saveData" onAfterDelete="saveData"> 
      <column label="title" name="title" type="DateTime" xid="default1"/>  
      <column label="completed" name="completed" type="Boolean" xid="default2"/>  
      <column label="id" name="id" type="String" xid="default3"/>  
      <rule xid="rule4"> 
        <readonly xid="readonly1"> 
          <expr xid="default9">js:$model.test()</expr>
        </readonly> 
      </rule>
    </div>  
    <div component="$UI/system/components/justep/data/data" xid="tempData"
      ...
      <column label="status" name="status" type="String" xid="default5"/>  
      <column label="completedCount" name="completedCount" type="Integer" xid="default6"/>  
      <column label="allCompleted" name="allCompleted" type="Boolean" xid="default7"/>  
      <column label="col3" name="col3" type="String" xid="default8"/>
    </div> 
  </div>  
  ...
  <div component="$UI/system/components/justep/list/list" class="x-list" xid="list1"
    data="todoData" filter="js:$model.tempData.val('status')=='All' || ($model.tempData.val('status')=='Active' &amp;&amp; !$row.val('completed')) || ($model.tempData.val('status')=='Completed' &amp;&amp; $row.val('completed'))"> 
      ...
      <li xid="li1"> 
        <div component="$UI/system/components/justep/row/row" class="x-row x-row-center top-line"
          ...
          <div xid="col4" style="padding: 10px 10px;" class="split-line"> 
            <span component="$UI/system/components/justep/button/checkbox" class="x-checkbox"
              xid="checkbox1" bind-ref="ref('completed')"/> 
          </div>  
          <div class="x-col" xid="col5"> 
            <div component="$UI/system/components/justep/output/output" class="x-output"
              xid="output1" bind-ref="ref('title')" bind-css="{'title-completed': val('completed')}"/> 
          </div>  
          <div xid="col6"> 
            <a component="$UI/system/components/justep/button/button" class="btn btn-link btn-sm btn-only-icon"
              label="" xid="deleteBtn" icon="icon-android-close" style="color:#FF0000;"
  ...
  <input component="$UI/system/components/justep/input/input" class="form-control"
    xid="input1" bind-ref="tempData.ref('col3')" style="width:813px;"/>
</div>
