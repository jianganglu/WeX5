...
(function($) {
	...
	Calendar.prototype._day = function(week, day) {
		...
		t.lunar_day = '';
		if(this.options.lunar_day){
			var l = this.options.lunar_day(curdate);
			t.lunar_day = l.term || l.lDate;
		}
		t.cls = cls;
		t.day = day;
	...
	Calendar.prototype._loadEvents = function() {
		...
		switch($.type(source)) {
			...
			default:
				loader = function() {
					var events = self.options.onEventsLoad.call(self, self.options.position.start, self.options.position.end, browser_timezone);
					return  events || [];
				};
		}
		if(!loader) {
	...
	Calendar.prototype._loadTemplate = function(name) {
		...
		/*
		var html = $("#tmpls [name='" + name + "']").text();
		this.options.templates[name] = _.template(html);
		 */
		var self = this;
		$.ajax({
	...
	Calendar.prototype._update = function() {
		...
		// TODO: 	$('*[data-toggle="tooltip"]').tooltip({container: 'body'});
	...
	function showEventsList(event, that, slider, self) {
		...
		var begin = new Date(parseInt(event_list.data('cal-start'))), 
			end = new Date(parseInt(event_list.data('cal-end')));
		self.options.onEventsSelected(begin, end, browser_timezone);
		return;
